...
void applyExpandedStates(List<bool> expandedState) {
  final expandedStateStr =
      List<String>.generate(expandedState.length, (index) => expandedState[index] ? "1" : "0")
          .join("");
...
void applyDetailSizes(List<int> expandedHeights) {
  final expandedHeightStr =
      List<String>.generate(expandedHeights.length, (index) => expandedHeights[index].toString())
          .join("");
  final prefService = Get.find<BasePrefService>();
