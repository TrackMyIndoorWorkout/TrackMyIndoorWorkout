...
void main() {
  ...
  group('Migration CSV imports identically', () {
    ...
    getRandomInts(smallRepetition, 300, rnd).forEach((recordCount) {
      ...
      test('$recordCount', () async {
        ...
        expect(
          importedActivity.start.millisecondsSinceEpoch,
        ...
        expect(
          importedActivity.trainingPeaksFileTrackingUuid,
          activity.trainingPeaksFileTrackingUuid,
        );
        ...
        for (final pairs in IterableZip<Record>([importedRecords, records.map((e) => e.record)])) {
          ...
          expect(
            pairs[0].timeStamp!.millisecondsSinceEpoch,
  ...
  group(
    'Migration CSV import is able to import unfinished activity and finish it as a side effect',
    () {
      final rnd = Random();
      getRandomInts(smallRepetition, 300, rnd).forEach((recordCount) {
        recordCount += 20;
        test('$recordCount', () async {
          ...
          final importedRecords = await DbUtils().getRecords(importedActivity.id);
          expect(importedRecords.length, records.length);
          for (final pairs in IterableZip<Record>([
            importedRecords,
            records.map((e) => e.record),
          ])) {
            expect(pairs[0].activityId, importedActivity.id);
            expect(
              pairs[0].timeStamp!.millisecondsSinceEpoch,
